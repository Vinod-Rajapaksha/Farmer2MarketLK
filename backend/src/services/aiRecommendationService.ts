...
export const recommendProduce = async (query: string) => {
  ...
  const availableProduce = await Produce.find({ status: ProduceStatus.AVAILABLE }).select(
    '_id name category quantity unit price district availableDate'
  );
  ...
  try {
    ...
    if (parsedData.recommendations && Array.isArray(parsedData.recommendations)) {
      const populatedRecommendations = await Promise.all(
        parsedData.recommendations.map(async (rec: any) => {
          const produce = await Produce.findById(rec.produceId).populate('farmerId', 'name phone district');
          return {
            produce,
    ...
    throw new Error('AI failed to process the request correctly. Please try again.');
  }
};
